 ...
 */
export function configureRequests(options: { delayMs?: number; userAgent?: string }): void {
 ...
 */
export async function fetchWithRetry(url: string, tries = 3, backoffMs = 400): Promise<Response> {
  let lastErr: unknown;
  for (let i = 0; i < tries; i++) {
    try {
      if (requestDelayMs > 0) {
        const jitter = Math.floor(Math.random() * Math.max(1, requestDelayMs * 0.2));
 ...
 */
export async function downloadBinary(url: string, dest: string, silent = false): Promise<boolean> {
  try {
